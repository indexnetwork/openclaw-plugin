...
Before doing anything else, check whether any Index Network MCP tool is already callable (for example `read_user_profiles`).

- If YES: the bootstrap is already done. Stop reading this file and follow the MCP server's own instructions.
...
- **`register_agent` fails**: re-check the personal agent key; the user may need to regenerate it at https://index.network/agents.
